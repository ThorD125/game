...
function Sprites:Quads()
    ...
    for i = 0, self.x_count - 1 do
        for j = 0, self.y_count - 1 do
            ...
            quads[self.quad_count] = love.graphics.newQuad(i * self.width, j * self.height, self.width, self.height,
                width, height)
        end
    end
...
function Sprites:render(quad_count_position, x, y)
    resetColor()
    love.graphics.draw(self.img, self.quads[quad_count_position] or self.quads[1], math.floor(x or self.x),
        math.floor(y or self.y), 0, self.x_scale, self.y_scale)
